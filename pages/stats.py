...
# Registrar la página con la ruta correcta
dash.register_page(__name__, path="/stats/general", name="Estadísticas del Equipo")
...
layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H5("Exportar Reporte", className="text-white mb-3"),
                    dbc.Button(
                        "Descargar PDF",
                        id="btn-export-pdf",
                        color="success",
...
def generar_pdf_reporte(tipo_sesion, n_sesiones):
    """
    Genera un PDF con los gráficos y estadísticas
    """
    try:
        # Cargar datos
        df = load_data()
        ...
        # Procesar datos según tipo de sesión
        if tipo_sesion == "Semana":
            df_ordenado = df.copy()
            if not pd.api.types.is_datetime64_dtype(df_ordenado['Date']):
                df_ordenado['Date'] = pd.to_datetime(df_ordenado['Date'], errors='coerce')
        ...
        # Título del reporte
        story.append(Paragraph(f"Reporte de Estadísticas del Equipo - {tipo_sesion}", title_style))
        story.append(Paragraph(f"Fecha del reporte: {datetime.now().strftime('%d/%m/%Y %H:%M')}", styles['Normal']))
        story.append(Paragraph(f"Últimas {n_sesiones} sesiones analizadas", styles['Normal']))
        story.append(Spacer(1, 20))
        
        # Generar gráficos y convertir a imágenes
        if sesiones_stats is not None and not sesiones_stats.empty:
            # Gráfico de distancia
            fig_distancia = graficar_distancia_sesion(sesiones_stats, n_sesiones, tipo_sesion)
            if fig_distancia:
                img_bytes = pio.to_image(fig_distancia, format="png", width=800, height=400)
                img = Image(io.BytesIO(img_bytes), width=6*inch, height=3*inch)
                
                story.append(Paragraph("Análisis de Distancia por Sesión", subtitle_style))
                story.append(img)
                story.append(Spacer(1, 20))
        
        if velocidad_stats is not None and not velocidad_stats.empty:
            # Gráfico de velocidad alta
            fig_velocidad = graficar_velocidad_alta(velocidad_stats, n_sesiones, tipo_sesion)
            if fig_velocidad:
                img_bytes = pio.to_image(fig_velocidad, format="png", width=800, height=400)
                img = Image(io.BytesIO(img_bytes), width=6*inch, height=3*inch)
                
                story.append(Paragraph("Análisis de Sprint Distance por Sesión", subtitle_style))
                story.append(img)
                story.append(Spacer(1, 20))
        ...
        if aceleraciones_stats is not None and not aceleraciones_stats.empty:
            ...
            if fig_aceleraciones:
                img_bytes = pio.to_image(fig_aceleraciones, format="png", width=800, height=400)
                img = Image(io.BytesIO(img_bytes), width=6*inch, height=3*inch)
                
                story.append(Paragraph("Análisis de Aceleraciones y Desaceleraciones", subtitle_style))
                story.append(img)
...
# Nuevo callback para la exportación PDF
@callback(
    [Output("download-pdf", "data"),
     Output("download-status", "children")],
    [Input("btn-export-pdf", "n_clicks")],
    [State("tipo-sesion-dropdown", "value"),
     State("n-sesiones-slider", "value")],
    prevent_initial_call=True
)
def exportar_pdf(n_clicks, tipo_sesion, n_sesiones):
    """
    Callback para exportar el reporte a PDF
    """
    if n_clicks is None:
        return None, ""
    ...
    try:
        # Generar PDF
        pdf_data = generar_pdf_reporte(tipo_sesion, n_sesiones)
        
        if pdf_data is None:
            return None, dbc.Alert("Error al generar el PDF. Verifique que hay datos disponibles.", color="danger", className="mt-2")
        
        # Crear nombre del archivo
        fecha_actual = datetime.now().strftime("%Y%m%d_%H%M")
        filename = f"reporte_estadisticas_{tipo_sesion}_{fecha_actual}.pdf"
        
        return dict(content=pdf_data, filename=filename, type="application/pdf"), \
               dbc.Alert("¡PDF generado exitosamente!", color="success", className="mt-2")
        
    except Exception as e:
        return None, dbc.Alert(f"Error al exportar PDF: {str(e)}", color="danger", className="mt-2")